...
def Best_Final_Rank(team,date,dates,matches,daily_scores,daily_rank_board):
	...
	for teami in daily_rank_board[0].keys():
		daily_score[teami] = daily_scores[teami][Num_of_date-1]


	Daily_Rank = daily_rank_board[Num_of_date-1]
	Best_score = daily_score
	Best_Rank = Daily_Rank
	#print Best_score[team],team#, Best_Rank
	while True:
...
def Best_daily_Rank(team,teams,Best_Rank,Best_score,daily_match):
	...
	for match in daily_match.keys():
		if match[0] == team:
			Best_score[team] = (Best_score[team][0] + 1, Best_score[team][1] )
			...
			win1 = Best_score[match[0]][0]
			lose1 = Best_score[match[0]][1]
			r2 = Best_Rank[match[1]]
			win2= Best_score[match[1]][0]
			lose2 = Best_score[match[1]][1]
			r = Best_Rank[team]
			win = Best_score[team][0]
			lose = Best_score[team][1]
			if r1 < r2:
				if r1 < r < r2 :
